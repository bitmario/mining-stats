...
MINER_TYPES = ((MINER_CLAYMORE, _("Claymore")),)


class Rig(models.Model):
    type = models.CharField(
        _("miner type"), max_length=10, choices=MINER_TYPES, default=MINER_CLAYMORE
    )
...
class MinerStatusManager(models.Manager):
    def create(self, *args, **kwargs):
        if "stats" in kwargs and isinstance(kwargs["stats"], miners.Stats):
            stats = kwargs["stats"]
            del kwargs["stats"]
            kwargs["miner"] = Rig.objects.get(
                host=stats.miner.host, port=stats.miner.port
            )
...
class MinerStatus(models.Model):
    ...
    error = models.CharField(
        _("error"), max_length=512, blank=True, null=True, default=None
    )
    ...
    hashrate = models.DecimalField(
        _("hashrate (MH/s)"), max_digits=9, decimal_places=3, blank=True, null=True
    )
    ...
    class Meta:
        ...
        indexes = [models.Index(fields=["miner", "created_at"])]


class GpuStatus(models.Model):
    miner_status = models.ForeignKey(
        MinerStatus,
        verbose_name=_("miner status record"),
        related_name="gpus",
...
class SiteConfiguration(SingletonModel):
    restrict_stats = models.BooleanField(
        _("restrict stats?"),
        default=False,
        help_text=_("restrict stats viewing to logged in users"),
    )
    restrict_actions = models.BooleanField(
        _("restrict actions?"),
        default=True,
        help_text=_("restrict actions (restart, etc.) to logged in users"),
    )
    warn_temp = models.PositiveIntegerField(
        _("warning temperature"),
        default=55,
        help_text=_("value above which stats are displayed in yellow"),
    )
    danger_temp = models.PositiveIntegerField(
        _("danger temperature"),
        default=70,
        help_text=_("value above which stats are displayed in red"),
    )
    warn_fan = models.PositiveIntegerField(
        _("warning fan %"),
        default=60,
        help_text=_("value above which stats are displayed in yellow"),
    )
    danger_fan = models.PositiveIntegerField(
        _("danger fan %"),
        default=75,
        help_text=_("value above which stats are displayed in red"),
    )
